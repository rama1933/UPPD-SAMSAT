...
class TrxPendaftaranService
{
    ...
    function getDataTrxPendaftaran($id = null, $user_id = null)
    {
        if ($id === null && $user_id == null) {
            $data = Pendaftaran::with('pendaftarans')->with('jenis')->with('merk')->with('type')->with('biodata')->get();
        } elseif ($id === null && $user_id != null) {
            $data = TrxPendaftaran::with('jenis')->with('merk')->with('type')->where('user_id', $user_id)->get();
        } else {
    ...
    {
        if ($id === null && $user_id == null) {
            $data = Pendaftaran::with('pendaftarans')->with('jenis')->with('merk')->with('type')->with('biodata')->whereHas('pendaftarans', function ($q) {
                $q->where('status', '1');
            })->get();
        } elseif ($id === null && $user_id != null) {
            $data = TrxPendaftaran::with('jenis')->with('merk')->with('type')->where('user_id', $user_id)->get();
